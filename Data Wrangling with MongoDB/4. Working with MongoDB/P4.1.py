...
def audit_fields(entry):
  
    #trim out redundant description in parenthesis from the 'rdf-schema#label' field, like "(spider)"
    if entry["label"]:
        entry["label"] = re.sub(r'\(.*?\)', '', entry["label"])
        
    #replaces all NULL or non-alphanumeric names in "name" with value in "label"
    if (entry["name"] == "NULL") or (not entry["name"].isalnum()):
        entry["name"] = entry["label"] 
    
    #converts all NULL fields to None 
    for field in entry:
        if entry[field] == "NULL":
            entry[field] = None
            
    # converts values in "synonym" to lists and cleans values by removing prefixes   
    if entry["synonym"]!=None:
        temp = entry["synonym"]
        temp = temp.strip("{}*")
        entry["synonym"] = temp.split("|")
    
    #strips whitespace from all fields
    
    fields = {"family", "class", "phylum", "order", "kingdom", "genus"}
    classification = {}
    for key in entry.keys():
        if entry[key]!=None and type(entry[key])!=list:
...
def process_file(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            ...
            entry = audit_fields(entry)
            data.append(entry)
            
            pass
    
    print pprint.pprint(data[0])
    return data
