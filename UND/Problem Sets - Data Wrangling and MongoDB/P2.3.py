    ...
    with open("{}/{}".format(datadir, f), "r") as html:
        ...
        for entry in table.find_all('tr'):
            ...
            for field in entry.find_all("td"):
                fields.append(field.get_text())
                    
            if fields[1]=="TOTAL" or fields[0]=="Year":
                continue
            info['year'] = int(fields[0])
            info['month'] = int(fields[1])
            info['flights']= {}
            info['flights']['domestic']= int(float(fields[2].replace(',', '')))
            info['flights']['international']= int(float(fields[3].replace(',', '')))
